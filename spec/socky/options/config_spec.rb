  ...
  context "class" do
    ...
    context "on read" do
      it "should raise error if file doesn't exists" do
        $stdout.should_receive(:write).with("You must generate a config file (socky -g filename.yml)")
        lambda { described_class.read("abstract") }.should raise_error SystemExit
      end
      it "should raise error if file is corrupted" do
        $stdout.should_receive(:write).with("Provided config file is invalid.")
    ...
    context "on generate" do
      it "should raise error if file exists" do
        $stdout.should_receive(:write).with("Config file already exists. You must remove it before generating a new one.")
      ...
      it "should raise error if path is not valid" do
        $stdout.should_receive(:write).with("Config file is unavailable - please choose another.")
      ...
      it "should create file and exit if path is valid" do
        ...
        begin
          $stdout.should_receive(:write).with("Config file generated at #{path}")
